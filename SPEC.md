# AAIP Protocol Specification

The canonical protocol specification lives at [`docs/aaip-spec.md`](docs/aaip-spec.md).

It covers:
- Agent identity format and keypair derivation
- Proof of Execution (PoE) schema and hash computation
- Validator voting protocol and consensus rule
- CAV audit trigger and reputation formula
- Escrow payment flow and fee distribution
- Fraud signal definitions and verdict mapping
- Full CLI and SDK reference

---

*AAIP v1.0 — MIT License — vuneum.com*
