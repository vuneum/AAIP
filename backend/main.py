...
async def metrics_endpoint() -> Response:
    ...
    global _request_count, _payment_count, _anchor_count
    
    current_time = time.time()
    uptime = current_time - _metrics_start_time
    
    # Read metrics under lock for consistency
    async with _metrics_lock:
        request_count = _request_count
        payment_count = _payment_count
        anchor_count = _anchor_count
    
    metrics_lines = [
        "# HELP aaip_uptime_seconds Total uptime of the AAIP service in seconds.",
        "# TYPE aaip_uptime_seconds gauge",
        f"aaip_uptime_seconds {uptime}",
        "",
        "# HELP aaip_requests_total Total number of HTTP requests processed.",
        "# TYPE aaip_requests_total counter",
        f"aaip_requests_total {request_count}",
        "",
        "# HELP aaip_payments_total Total number of payment transactions processed.",
        "# TYPE aaip_payments_total counter",
        f"aaip_payments_total {payment_count}",
        "",
        "# HELP aaip_anchors_total Total number of proof anchors executed.",
        "# TYPE aaip_anchors_total counter",
        f"aaip_anchors_total {anchor_count}",
        "",
        "# HELP aaip_info AAIP service information.",
        "# TYPE aaip_info gauge",
        f'aaip_info{{version="1.0.0", service="AAIP"}} 1',
    ]
...
async def increment_payment_count():
    ...
    global _payment_count
    async with _metrics_lock:
        _payment_count += 1


async def increment_anchor_count():
    """Increment the anchor counter."""
    global _anchor_count
    async with _metrics_lock:
        _anchor_count += 1
