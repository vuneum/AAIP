...
from tracing import trace_span_async
...
async def submit_poe_trace(
    ...
    async with trace_span_async(
        ...
        attributes={
            ...
            "task_id": request.trace.task_id if request.trace else "unknown",
            "has_submitted_hash": bool(request.poe_hash or (request.trace and request.trace.poe_hash))
        }
    ) as span:
        trace = request.trace
        ...
        if not agent:
            span.set_status("error", f"Agent not found: {request.agent_id}")
        ...
        # Set span attributes
        span.set_attribute("trace_id", str(record.id))
        span.set_attribute("hash_verified", hash_verified)
        span.set_attribute("verdict", verdict)
        span.set_attribute("fraud_flag_count", len(fraud_flags))
        span.set_attribute("step_count", len(trace.steps))
        span.set_attribute("duration_ms", duration_ms)
        
        if verdict == "verified":
            span.set_status("success")
        elif verdict == "suspicious":
            span.set_status("warning", f"Fraud flags detected: {fraud_flags}")
        else:
            span.set_status("error", f"Invalid trace: hash_verified={hash_verified}")
