...
class LedgerEntry:
    entry_id:   str
    agent_id:   str
    entry_type: str   # charge|credit|fee|refund|penalty|validator_reward
    amount:     float
    currency:   str   = "USDC"
...
class EscrowEngine:
    ...
    def settle_task(
        ...
        validator_reward = round(task.value * self.VALIDATOR_REWARD_FRACTION, 6)
        fraud_penalty    = 0.0
        disputed         = False
        ...
        if consensus.fraud_detected:
            ...
            fraud_penalty = round(
                task.value * self.FRAUD_PENALTY_MULTIPLIER, 6
            )
            executor_credit = 0.0
            requester_refund = task.value

            self._add_entry(state, task.requester_id, "refund",
                            +requester_refund, task.task_id)
            self._add_entry(state, task.executor_id, "penalty",
            ...
            self._add_entry(state, task.executor_id, "credit",
        ...
        self._add_entry(state, "protocol", "fee", +protocol_fee, task.task_id)
    ...
    def _distribute_validator_rewards(self, total_reward, consensus, state, task_id):
        ...
        for vid in voters:
            ...
            self._add_entry(state, vid, "validator_reward", +per_validator, task_id)
