...
import os as _os
_ETH_USD_RATE = float(_os.environ.get("AEP_ETH_USD_RATE", "3500.0"))
PROTOCOL_FEE_RATE = float(_os.environ.get("AEP_PROTOCOL_FEE_RATE", "0.02"))
PROTOCOL_TREASURY = _os.environ.get("AEP_TREASURY_ADDRESS", "0x000000000000000000000000000000000000dEaD")

# Auto-bill when accumulated USD spend hits this threshold
_AUTO_BILL_THRESHOLD_USD = float(_os.environ.get("AEP_BILL_THRESHOLD_USD", "1.0"))
...
def calculate_cost(
    ...
    cost_usd  = (
        rates.get("per_call", 0.0)
        + tokens_in  / 1000.0 * rates.get("per_1k_in",  0.002)
        + tokens_out / 1000.0 * rates.get("per_1k_out", 0.008)
    )
    cost_eth  = cost_usd / _ETH_USD_RATE
    return round(cost_usd, 8), round(cost_eth, 12)
...
class UsageMeter:
    ...
    def __init__(
        ...
        pricing: dict | None = None,
        auto_bill_threshold_usd: float = _AUTO_BILL_THRESHOLD_USD,
    ) -> None:
