...
mod tests {
    ...
    fn test_client_new_with_key() {
        let client = AAIPClient::new("test-api-key-abc", None);
        // If new() panics or returns an unusable value the test fails.
    ...
    fn test_client_new_with_base_url() {
        let client = AAIPClient::new("key", Some("https://custom.api.example.com"));
        let _ = client;
    }

    #[test]
    fn test_client_new_empty_key() {
        // Empty key is technically valid at construction time —
        // rejection happens at the API call level.
        let client = AAIPClient::new("", None);
        let _ = client;
    }

    // ── Unit: AgentManifest serialisation ─────────────────────────────────────

    #[test]
    fn test_manifest_serialises_to_json() {
        let manifest = AgentManifest {
            agent_name: "RustTestAgent".to_string(),
            owner: "TestOrg".to_string(),
            endpoint: "https://api.example.com/agent".to_string(),
            capabilities: vec!["code_review".to_string(), "summarisation".to_string()],
            framework: Some("custom".to_string()),
            ..Default::default()
        };

        let json = serde_json::to_string(&manifest).expect("serialisation failed");
        assert!(json.contains("RustTestAgent"));
        assert!(json.contains("code_review"));
    }

    #[test]
    fn test_manifest_round_trips() {
        let original = AgentManifest {
            agent_name: "RoundTrip".to_string(),
        ...
        let decoded: AgentManifest = serde_json::from_str(&json).unwrap();
        assert_eq!(original.agent_name, decoded.agent_name);
    ...
    fn test_error_display_auth() {
        let err = AAIPError::Auth;
        let msg = format!("{}", err);
        assert!(msg.to_lowercase().contains("auth") || msg.to_lowercase().contains("key"));
    }

    #[test]
    fn test_error_display_not_found() {
        let err = AAIPError::NotFound("agent-xyz".to_string());
        let msg = format!("{}", err);
        assert!(msg.contains("agent-xyz"));
    }

    #[test]
    fn test_error_display_api() {
        let err = AAIPError::Api { status: 429, message: "rate limited".to_string() };
        let msg = format!("{}", err);
